-- ============================================================
-- AURÈS CÉRAMIQUE — Supabase schema (v2: boutique COD)
-- Paste this whole file into: Supabase Dashboard > SQL Editor > New query > Run
-- Safe to run again: policies and functions are dropped and recreated,
-- your data (pieces, categories, inquiries, orders, settings) is kept.
--
-- v2 adds the shop engine from the second site while keeping the
-- atelier story: prices + stock on products, COD orders with 58 wilayas
-- (home / stopdesk), promo codes, free-delivery threshold, order tracking.
-- Existing installs: new columns/tables are added, nothing is overwritten.
--
...
create table if not exists products (
  ...
  colors text[] not null default '{}',        -- available colors / designs = variants
  dimensions text not null default '',         -- e.g. 'Ø 27 cm'
  material text not null default '',           -- e.g. 'Glazed clay'
...
-- v2 shop columns (safe on existing installs)
alter table products add column if not exists price numeric not null default 0 check (price >= 0);
alter table products add column if not exists compare_at_price numeric check (compare_at_price >= 0);
alter table products add column if not exists stock int not null default 0 check (stock >= 0);

-- visitors asking for information — kept from v1 (contact-driven side).
...
-- ---------- ORDERS (cash on delivery only, v2) ----------
create table if not exists orders (
  ...
  items jsonb not null,              -- [{product_id, name_*, color, price, qty, photo}]
...
alter table inquiries  alter column id set generated by default;
...
-- ============================================================
-- SUBMITTING AN INQUIRY (kept from v1 — contact / atelier story)
-- Visitors cannot insert into `inquiries` directly — they call
-- submit_inquiry(), which validates and rate-limits (5 per hour per phone).
-- ============================================================
drop function if exists public.submit_inquiry(text,text,text,text,bigint);

create or replace function public.submit_inquiry(
  p_name text, p_phone text, p_email text, p_message text, p_product_id bigint
)
returns jsonb language plpgsql security definer set search_path = public, pg_temp
as $$
declare
  clean text; v_id bigint;
begin
  p_name    := left(btrim(coalesce(p_name,'')), 120);
  p_email   := left(btrim(coalesce(p_email,'')), 160);
  p_message := left(btrim(coalesce(p_message,'')), 1000);

  if p_name = '' then raise exception 'MISSING_NAME'; end if;

  clean := dz_phone(p_phone);
  if clean is null then raise exception 'INVALID_PHONE'; end if;

  -- light anti-spam on the open form
  if (select count(*) from inquiries
      where phone = clean and created_at > now() - interval '1 hour') >= 5 then
    raise exception 'TOO_MANY';
  end if;

  insert into inquiries (name, phone, email, message, product_id)
  values (p_name, clean, p_email, p_message,
          case when p_product_id is null then null
               else (select id from products where id = p_product_id and active) end)
  returning id into v_id;

  return jsonb_build_object('ok', true, 'id', v_id);
end $$;

revoke all on function public.submit_inquiry(text,text,text,text,bigint) from public;
grant execute on function public.submit_inquiry(text,text,text,text,bigint) to anon, authenticated;

-- ============================================================
-- PLACING AN ORDER (v2 — COD, same engine as the second site)
-- Visitors cannot insert into `orders` directly. place_order() rebuilds the
-- basket from the real products table and ignores every price the browser
-- sent. Ceramics difference: the variant is `color` (from products.colors),
-- not a clothing size. Stock is reserved in the same transaction.
-- Pricing: 1) global sale → 2) promo code → 3) free delivery threshold.
-- ============================================================
...
begin
  ...
  for line in select * from jsonb_array_elements(p_items) loop
    ...
    -- ceramics variant: color must be one of products.colors when the piece
    -- declares colors. Accepts legacy `size` key from older clients too.
    v_color := coalesce(line->>'color', line->>'size', '');
    if coalesce(array_length(prod.colors, 1), 0) > 0
       and not (v_color = any (prod.colors)) then
    ...
    where dz_phone(o.phone) = p_phone
      ...
      and not exists (
        ...
        left join jsonb_array_elements(o.items) x
          ...
          and coalesce(x->>'color', x->>'size', '') = coalesce(c->>'color', '')
...
-- inquiries: no direct access at all — submit_inquiry() is the only door.
-- orders: no direct access at all — place_order()/track_order() only.
-- ============================================================
...
drop policy if exists "owner all inquiries" on inquiries;
...
create policy "read public settings" on settings
  for select using (key in ('store','hero','whatsapp','announce','socials',
                            'address','verifications',
                            'zones','promo','free_delivery_from'));

create policy "owner all categories" on categories
  for all using (public.is_owner()) with check (public.is_owner());
create policy "owner all products" on products
  for all using (public.is_owner()) with check (public.is_owner());
create policy "owner all inquiries" on inquiries
  for all using (public.is_owner()) with check (public.is_owner());
...
-- ---------- Storage: product & gallery photos ----------
